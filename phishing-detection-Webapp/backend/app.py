...
try:
    model = joblib.load(r"C:\Users\arshi\OneDrive\Desktop\Mini Project\Website\phishing-detection-app\backend\machine_learning\model\phishing_model.pkl")
    scaler = joblib.load(r"C:\Users\arshi\OneDrive\Desktop\Mini Project\Website\phishing-detection-app\backend\machine_learning\model\scaler.pkl")
except:
...
def extract_features(url):
    ...
    # ip_in_url = 1 if re.match(r"^(?:\d{1,3}\.){3}\d{1,3}", url) else 0
    subdomainLength = len(ext.subdomain)
    phishing_keywords = ["secure", "account", "login", "bank", "verify", "update"]
    keyword_flag = any(word in url.lower() for word in phishing_keywords)
    encoded_chars = url.count("%")
    dot_to_length_ratio = url.count('.') / len(url) if len(url) > 0 else 0
    ...
    return [
        ...
        url.count('.'),
        sum_upper,
        sum_upper / len(url) if len(url) > 0 else 0,
        subdomainLength,
        keyword_flag,
        encoded_chars,
        dot_to_length_ratio
...
#         print(f"WHOIS data for {domain}: {domain_info}")
#         if domain_info and (domain_info.get('creation_date') or domain_info.get("registrar")):
#             return True
#         else:
#             return False
...
def predict():
    ...
    #Checking for domain informations and return Legitimate if domain registration infos are found
    # if check_domain(url):
